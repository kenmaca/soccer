...
double Ki = 0;
double Kd = 0;
double a[2],b[2],c[2],d[2],x[2], sp = 0.05; 
double limit = 3.0, *dir;
double des_angle[2], des_pos[2];
double prev_ori[2];
int fixed = 0;
double goal[2];
double off_set = 0.3;
...
void id_bot(struct RoboAI *ai, struct blob *blobs)
{
 ...
 drive_speed(50);			// Start forward motion to establish heading
 ...
 **************************************************************************/
 ...
 if (ai->st.state==0||ai->st.state==100||ai->st.state==200)  	// Initial set up - find own, ball, and opponent blobs
 {
  ...
  {				// if robot identification is successful.
   ...
   all_stop();
   clear_motion_flags(ai);
   prev_ori[0] = ai->st.self->mx;
   prev_ori[1] = ai->st.self->my;
   goal[0] = (ai->st.side) ? 0 : 1024;
   goal[1] = 384;
  ...
  if (ai->st.selfID) { 
   ...
   if (ai->st.state >= 101 && ai->st.state < 200) {
    ...
    if (ai->st.state == 101 && ai->st.ballID) {
     ...
     des_pos[0] = ai->st.self->cx;
     des_pos[1] = ai->st.ball->cy;
     drive_to_straight(ai, des_pos, des_angle);
    }  else if (ai->st.state == 103 && ai->st.ballID) {
     des_angle[0] = (ai->st.side) ? -1 : 1;
     des_angle[1] = 0;
     rotate(ai, des_angle);
    } else if (ai->st.state == 104 && ai->st.ballID) {
     des_pos[0] = ai->st.ball->cx - des_angle[0] * 150;
     des_pos[1] = ai->st.ball->cy;
     printf("%f\n", ai->st.self->cx - ai->st.ball->cx);
     drive_to_straight(ai, des_pos, des_angle);
    ...
    prev_ori[0] = fixer(ai) * ai->st.self->dx;
    prev_ori[1] = fixer(ai) * ai->st.self->dy;
  }
...
void getSplinCoef (struct RoboAI *ai) {

  int n = 3,i,j,k;
  float h[10], sum, s[10] = {0}, F[10], f[10], p, m[10][10] = {0}, temp;
  float x0, x1, x2, y0, y1, y2, p1, p2, p3, p4, l1, l2, l3, l4;


  // get the third point to do the spline interpolation
  x0 = ai->st.self->cx;
  y0 = ai->st.self->cy;
  ...
  x1 = ((p1*l2 - l1*p2)*(p3 - p4) - (p1 - p2)*(p3*l4 - l3*p4)) / ((p1 - p2)*(l3 - l4) - (l1 - l2)*(p3 - p4));
  y1 = ((p1*l2 - l1*p2)*(l3 - l4) - (l1 - l2)*(p3*l4 - l3*p4)) / ((p1 - p2)*(l3 - l4) - (l1 - l2)*(p3 - p4));
  ...
  // Form the matrix and the two vectors
  for(i = 1; i < n-1; i++) {
  ...
  for(i=0;i<n-1;i++) {
    ...
    a[i] = (s[i+1] - s[i]) / (6*h[i]);
    b[i] = s[i]/2;
    c[i] = (f[i+1] - f[i]) / h[i] - (2*h[i]*s[i] + s[i+1]*h[i])/6;
    d[i] = f[i];
...
double splineFunc(double p) {

  int i, n = 3;
  for(i=0;i<n-1;i++) {
    if (x[i] <= p && x[i+1] >= p) {
      return a[i]*pow((p-x[i]),3) + b[i]*pow((p-x[i]),2)+c[i]*(p-x[i])+d[i];
    }
  }
  return -1;
}
...
double pid_update(struct PID* pid, double error) {

    double err_diff = (error - pid->prev_er)/pid->period;
    pid->er_sum += error*pid->period;

    pid->prev_er = error;



    return pid->Kp*error + pid->Kd*err_diff + pid->Ki*pid->er_sum;
}
...
double * getDir(struct RoboAI *ai, double dest_x, double dest_y) {
 ...
 static double di[2];
 if (fabs(ai->st.self->cx - dest_x) > limit || fabs(ai->st.self->cy - dest_y) > limit) {
  di[0] = dest_x - ai->st.self->cx;
  di[1] = dest_y - ai->st.self->cy;
 } else {
  di[0] = 1;
  di[1] = 1;
 }
...
void dirToPower(struct RoboAI *ai, double *dest) {
 if ((ai->st.self->cx*dest[1] - ai->st.self->cy*dest[0]) > 0) turn_left_speed(20);
...
int fixer(struct RoboAI *ai) {
  if ((prev_ori[0]*ai->st.self->dx + prev_ori[1]*ai->st.self->dy) > 0) return 1;
  else return -1;
}
